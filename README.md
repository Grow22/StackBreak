...
**Stack & Break** 는 두 명의 플레이어가 각기 다른 장르로 맞붙는 **1:1 경쟁 멀티플레이어** 터미널 게임입니다.
...
<p align="center">
  ...
  <sub>💣 <b>폭탄</b> — "BOMB!" 전체 화면 폭발로 주변 블록을 파괴</sub>
</p>

| ⛏️ 드릴 — 블록 관통 | 🛡 방패 — 무적 (이때 공격하면 공격자 기절) | 🔫 총 — 천장의 공격자 공격 |
| :---: | :---: | :---: |
...
### 🆚 1:1 PvP 대전
위에서는 블록이 쉴 새 없이 쏟아지고, 아래에서는 캐릭터가 그 틈을 헤집고 다닙니다. 공격자는 테트리스 블록을 떨어뜨려 방어자를 깔아뭉개려 하고, 방어자는 무너지는 보드 속에서 살아남아 천장의 공격자를 노립니다. **같은 보드 위에서 서로 전혀 다른 방식으로 맞붙는 대결**입니다.
...
### 👾 공격자 공략 (천장 보스전)
공격자는 보드 맨 위(천장)에 **보스(인베이더) 형태**로 등장합니다 (게임 화면에는 `Atk. HP`로 표시). 방어자가 **총(G)** 아이템으로 총알을 발사해 공격자 HP **5**를 0으로 만들면 방어자 승리입니다. 총알 속도는 틱당 2칸이며, 공격자와 겹친 총알만 데미지를 줍니다.
