...
| ⛏️ 드릴 — 블록 관통 | 🛡 방패 — 무적 (공격자 기절) | 🔫 총 — 천장 보스 공격 |
| :---: | :---: | :---: |
...
### 🆚 1:1 PvP 대전
위에서는 블록이 쉴 새 없이 쏟아지고, 아래에서는 캐릭터가 그 틈을 헤집고 다닙니다. 공격자는 테트리스 블록을 떨어뜨려 방어자를 깔아뭉개려 하고, 방어자는 무너지는 보드 속에서 살아남아 천장의 보스를 노립니다. **같은 보드 위에서 서로 전혀 다른 방식으로 맞붙는 대결**입니다.
...
### 👾 보스 시스템
보드 맨 위(천장)에 보스가 자리잡고 있습니다. 방어자가 **총(G)** 아이템으로 총알을 발사해 보스 HP **5**를 0으로 만들면 방어자 승리입니다. 총알 속도는 틱당 2칸이며, 보스 위치와 겹친 총알만 데미지를 줍니다.
